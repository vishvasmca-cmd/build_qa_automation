...
class HomePage:
    ...
    def your_basket_has_1_items_in_it_link(self):
        ...
        return self.page.get_by_role("link", name="Your basket has 1 items in it", exact=True).first

    @property
    def search_button(self):
        """The goal is to reach the checkout page. The history shows that I have already added an item to the c"""
        return self.page.get_by_role("button", name="Search", exact=True).first
...
class DysonIndiaOfficialWebsitePage:
    ...
    def save_16_000_dyson_v8_absolute__link(self):
        ...
        return self.page.get_by_role("link", name="Save ?16,000 Dyson V8 Absolute vacuum cleaner 4.4 stars out of 5 from 32104 Reviews 32104 ?27,900.", exact=True).first
...
class ProductsPage:
    ...
    def continue_to_basket_button(self):
        ...
        return self.page.get_by_role("button", name="Continue to basket", exact=True).first

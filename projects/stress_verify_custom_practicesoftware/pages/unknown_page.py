...
class UnknownPage:
    ...
    @property
    def password_input_login(self):
        """Input field for password in the login section"""
        return self.page.get_by_label('Password', exact=True).or_(self.page.locator('#password'))
    ...
    @property
    def lost_your_password_link(self):
        """Link to reset password"""
        return self.page.get_by_text('Lost your password?').or_(self.page.locator('a[href*="lost-password"]'))

    @property
    def email_address_input_register(self):
        """Input field for email address in the registration section"""
        return self.page.get_by_label('Email address').or_(self.page.locator('#reg_email'))

    @property
    def password_input_register(self):
        """Input field for password in the registration section"""

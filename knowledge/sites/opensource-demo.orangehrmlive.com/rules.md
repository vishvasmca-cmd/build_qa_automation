...
- ✅ PREFERRED: DO define locators using static strings or f-strings that are constructed with known variables at the time of locator definition. If dynamic values are needed, pass them as arguments to the method that uses the locator.

- ⚠️ PROHIBITED: DON'T use `eval()` on locator strings that contain references to Playwright objects like `page`. `eval()` can lead to scope issues and security vulnerabilities.

- ✅ PREFERRED: DO define locators as static strings or use f-strings to dynamically construct them, passing in the necessary variables directly.  Ensure the `page` object is available in the scope where the locator is used.
...
- ✅ PREFERRED: DO define locators as simple, static strings or use parameterized locators with proper string formatting or f-strings to inject dynamic values. Ensure all necessary variables are within the scope where the locator is used.

- ⚠️ PROHIBITED: DON'T use `eval()` to construct Playwright locators, especially when trying to inject the `page` object. This leads to scope issues and makes the locator brittle and hard to debug.

- ✅ PREFERRED: DO define locators as static strings or use f-strings for simple variable substitution. For more complex scenarios, use Playwright's built-in locator methods (e.g., `locator.locator('xpath=...')`) to build locators dynamically based on other locators or page state.
...
- ⚠️ PROHIBITED: DON'T use `eval()` within locator strings, especially when referencing page-specific objects like `page` itself. This creates scope issues and makes locators brittle and hard to maintain.

- ✅ PREFERRED: DO define locators as simple, static strings or use parameterized locators with proper string formatting. Pass the `page` object as an argument to methods that use the locators, ensuring the `page` object is always in scope.

- ⚠️ PROHIBITED: DON'T use `eval()` to construct locators, especially when they depend on variables defined outside the string. This makes debugging difficult and introduces security risks.

- ✅ PREFERRED: DO define locators as static strings or use f-strings to inject variables directly into the locator string. Ensure all variables used in the locator are properly scoped.

- ⚠️ PROHIBITED: DON'T use `eval()` with Playwright locators, especially when the locator string contains references to variables (like `page`) that are not available in the evaluation scope. This leads to unpredictable behavior and errors.

- ✅ PREFERRED: DO construct Playwright locators using f-strings or other string formatting methods to inject variables directly into the locator string *before* passing it to `page.locator()`. This ensures that all necessary variables are in scope.

- ⚠️ PROHIBITED: DON'T use `eval()` within locator strings, especially when referencing page-specific objects. This creates scope issues and makes debugging difficult.

- ✅ PREFERRED: DO define locators using f-strings or other string formatting methods to inject variables, ensuring the variables are properly scoped and accessible.

- ⚠️ PROHIBITED: DON'T use `eval()` with Playwright locators, especially when the locator string depends on variables defined outside the string itself. `eval()` can lead to scope issues and security vulnerabilities.

- ✅ PREFERRED: DO use Playwright's built-in locator methods (e.g., `page.locator()`, `page.getByRole()`, `page.getByText()`) with properly formatted strings or functions that directly access the `page` object within their scope. Consider using f-strings for dynamic locator construction.

- ⚠️ PROHIBITED: DON'T use `eval()` with Playwright locators, especially when the locator string contains references to variables (like `page`) that are not available in the evaluation scope. This leads to unpredictable errors and makes debugging difficult.

- ✅ PREFERRED: DO use f-strings or string formatting to construct dynamic Playwright locators. This ensures that variables are properly interpolated into the locator string at runtime and avoids scope issues.

- ⚠️ PROHIBITED: DON'T use `eval()` to construct locators, especially when they rely on variables defined outside the string itself. This makes the code brittle and hard to debug.

- ✅ PREFERRED: DO define locators as static strings or use f-strings to inject variables directly into the locator string during construction. This ensures that the necessary variables are in scope and avoids runtime evaluation errors.
...
- ⚠️ PROHIBITED: DON'T use `eval()` on locator strings, especially when they contain references to variables defined outside the string's scope. This makes the code brittle and prone to `NameError` exceptions.

- ✅ PREFERRED: DO construct locators using f-strings or other string formatting methods to inject variables safely and predictably into the locator string. This avoids the need for `eval()` and improves code readability and maintainability.

- ⚠️ PROHIBITED: DON'T use `eval()` within locator strings, especially when attempting to reference page-specific objects like `page`. This creates scope issues and makes locators brittle and hard to maintain.

- ✅ PREFERRED: DO define locators using static strings or f-strings that incorporate class attributes or configuration values. If dynamic behavior is needed, manipulate the locator string outside of the locator definition itself.

- ⚠️ PROHIBITED: DON'T use `eval()` within locator strings, especially when trying to reference page-specific objects. This creates scope issues and makes the locator brittle and hard to debug.

- ✅ PREFERRED: DO define locators using static strings or f-strings that are constructed with known variables at the time of locator definition. Use Playwright's built-in locator methods for dynamic element selection.

- ⚠️ PROHIBITED: DON'T use `eval()` with Playwright locators, especially when the locator string contains references to Playwright objects like `page`. This creates scope issues and makes the code harder to debug.

- ✅ PREFERRED: DO use parameterized locators or locator factories to dynamically construct locators based on the `page` object. This ensures that the `page` object is correctly scoped and accessible when the locator is evaluated.

- ⚠️ PROHIBITED: DON'T use `eval()` with Playwright locators, especially when trying to inject page-specific variables. This leads to scope issues and makes debugging difficult.

- ✅ PREFERRED: DO define locators as static strings or use f-strings for dynamic locator creation, ensuring all variables are properly scoped within the class or function where the locator is used.

- ⚠️ PROHIBITED: DON'T use `eval()` within locator strings, especially when attempting to reference page-specific objects. This creates scope issues and makes locators brittle and hard to maintain.

- ✅ PREFERRED: DO define locators as simple, static strings or use parameterized locators with proper variable substitution outside of `eval()` calls. Use f-strings or other string formatting methods to inject dynamic values into locators.

- ⚠️ PROHIBITED: DON'T use `eval()` on locator strings, especially when they contain references to variables defined outside the string's scope. This leads to unpredictable behavior and scope issues.

- ✅ PREFERRED: DO define locators as static strings or use f-strings to inject variables directly into the locator string during its creation, ensuring proper scope and avoiding runtime evaluation errors.

- ⚠️ PROHIBITED: DON'T use `eval()` with Playwright locators, especially when the locator string contains references to the `page` object or other variables defined outside the string's scope. This leads to unpredictable behavior and scope issues.

- ✅ PREFERRED: DO use Playwright's built-in locator methods and string formatting (f-strings or .format()) to construct dynamic locators. This ensures proper context and avoids the pitfalls of `eval()`.

- ⚠️ PROHIBITED: DON'T use `eval()` within locator strings to reference page-specific objects like `page`. Locators should be static or parameterized using values passed from the test, not by directly referencing the page object within the locator definition.

- ✅ PREFERRED: DO define locators as static strings or use parameterized locators with f-strings or similar methods to inject values from the test context. Ensure locators are robust and not dependent on runtime evaluation of page-specific objects.

- ⚠️ PROHIBITED: DON'T use `eval()` with Playwright locators, especially when the locator string contains references to Playwright objects like `page`. `eval()` can lead to scope issues and security vulnerabilities.

- ✅ PREFERRED: DO use Playwright's built-in locator methods (e.g., `page.locator()`, `page.getByRole()`, `page.getByText()`) directly with properly formatted locator strings or functions that receive the `page` object as an argument.

- ⚠️ PROHIBITED: DON'T use `eval()` on locator strings, especially when they contain references to variables defined outside the string's scope. This makes the code brittle and hard to debug.
...
- ⚠️ PROHIBITED: DON'T use `eval()` within locator strings, especially when attempting to reference page-specific objects like `page` itself. This creates scope issues and makes locators brittle and difficult to maintain.

- ✅ PREFERRED: DO define locators using static strings or f-strings that incorporate class attributes or configuration values. If dynamic behavior is needed, manipulate the locator string outside of the locator definition itself.

- ⚠️ PROHIBITED: DON'T use `eval()` with Playwright locators, especially when trying to inject the `page` object into the locator string. This leads to scope issues and makes the locator brittle and hard to debug.

- ✅ PREFERRED: DO define locators as static strings or use f-strings for dynamic parts that don't involve the `page` object directly.  Pass the `page` object to the `locator()` method.

- ⚠️ PROHIBITED: DON'T use `eval()` within locator strings, especially when attempting to reference page-specific objects. This creates scope issues and makes the locator brittle and difficult to maintain.
...
- ⚠️ PROHIBITED: DON'T use `eval()` to construct Playwright locators, especially when trying to inject the `page` object. This leads to scope issues and makes the code harder to debug.

- ✅ PREFERRED: DO use Playwright's built-in locator methods (e.g., `page.locator()`, `page.getByRole()`, `page.getByText()`) with properly formatted CSS selectors, XPath expressions, or text-based selectors. Pass the locator string directly to these methods.

- ⚠️ PROHIBITED: DON'T use `eval()` with locator strings that depend on the `page` object's scope. This creates a dependency on the execution context and makes the locator brittle.

- ✅ PREFERRED: DO define locators as static strings or use string formatting to inject variables into the locator string *before* passing it to the `locator()` method. This ensures the locator is properly constructed and avoids scope issues.
...
- ⚠️ PROHIBITED: DON'T assume that elements are immediately available after navigation; always account for potential loading delays.

- ✅ PREFERRED: DO implement explicit waits or assertions to ensure elements are present and interactable before attempting to interact with them.

- ⚠️ PROHIBITED: DON'T use locators that are not specific enough and can match multiple elements on the page. ALWAYS aim for unique and unambiguous locators.

- ✅ PREFERRED: DO use more specific locators, such as role-based locators (e.g., `get_by_role('button', name='Login')`) or locators that include attributes or classes to uniquely identify the target element.

- ⚠️ PROHIBITED: DON'T assume that navigation is complete immediately after a click. ALWAYS wait for the target element to be visible or the expected page content to load.

- ✅ PREFERRED: DO use `page.wait_for_load_state('networkidle')` or `page.wait_for_selector()` after navigation to ensure the page is fully loaded before interacting with elements.
...
- ✅ PREFERRED: DO construct Playwright locators directly using the `page.locator()` method with properly formatted CSS selectors, XPath expressions, or text-based selectors. Pass any variables needed for dynamic locators directly to the locator method.

- ⚠️ PROHIBITED: NEVER use `eval()` within locator strings to reference page objects or other variables. This creates scope issues and makes locators brittle and difficult to maintain.

- ✅ PREFERRED: ALWAYS define locators as static strings or use f-strings to dynamically construct them using known variables at the time of definition. Pass the page object to the locator when interacting with it.

- ⚠️ PROHIBITED: DON'T use `eval()` to construct locators, especially when they rely on variables defined outside the string. This can lead to scope issues and make debugging difficult.

- ✅ PREFERRED: DO use Playwright's built-in locator methods (e.g., `page.locator()`, `page.getByRole()`, `page.getByText()`) with properly formatted strings or functions to define locators. Ensure the `page` object is correctly passed to the locator.
